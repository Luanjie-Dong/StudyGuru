...
storage_url = "http://127.0.0.1:5007/upload" #POST
ocr_url = "http://127.0.0.1:5000/generate_topics" #POST
notes_API_url = "http://127.0.0.1:5005/notes" #POST
...
def handle_upload():
    ...
    try:
        ...
        print("File received, processing file upload now...")
        print(f"\nInvoking storage microservice...")
        ...
        print('Trigerring backend ocr and notes services in background...')
...
def handleNoteUpload(note_URL, module_id, course_id):
    if not note_URL or not module_id or not course_id:
        return print("error: Missing note_URL, module_id or course_id")
    
    try:
        print(f"\nInvoking OCR microservice...")
...
if __name__=='__main__':
    print("This is flask for " + os.path.basename(__file__) + ": handle_upload ...")
